...
conda create -n tangram1  python=3.10.14  -c https://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/conda-forge

conda activate tangram1

conda install scanpy==1.10.2  -c https://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/conda-forge
...
#adata_st.write("adata_st.h5ad")
adata_st =sc.read("adata_st.h5ad")
...
#img.save("fluo_image")
img =sq.im.ImageContainer().load("fluo_image")
...
#包含了每个分组中差异表达最显著的基因名称。
adata_sc.uns["rank_genes_groups"]["names"] 

markers_df = pd.DataFrame(adata_sc.uns["rank_genes_groups"]["names"]).iloc[0:100, :]
#markers_df.melt() 将数据框“拉平”成一个长格式，这样所有的基因名都会在一列中。
#np.unique() 用于获取所有基因名称中的唯一值。最后，将这些唯一的基因名称转换为一个列表 markers。
...
找到文件： D:\miniconda\envs\squidpy\lib\site-packages\tangram\mapping_utils.py:266
...
if isinstance(density_prior, str) and density_prior in ["rna_count_based", "uniform"]:
        if density_prior == "rna_count_based":
            density_prior = adata_sp.obs["rna_count_based_density"]
