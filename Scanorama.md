...
adata_spatial_anterior = sc.datasets.visium_sge(
    sample_id="V1_Mouse_Brain_Sagittal_Anterior") # 数据集1
...
adata_spatial_posterior = sc.datasets.visium_sge(
    sample_id="V1_Mouse_Brain_Sagittal_Posterior") # 数据集2
...
for name, adata in [
    ...
    sns.distplot( adata.obs["n_genes_by_counts"][adata.obs["n_genes_by_counts"] < 4000],
...
os.getcwd()
counts = pd.read_csv("GSE115746_cells_exon_counts.csv", index_col=0).T
meta = pd.read_csv(
        "GSE115746_complete_metadata_28706-cells.csv", index_col="sample_name")
...
adata_cortex.write_h5ad("adata_processed.h5ad")
...
adata_posterior_subset_transfer.write_h5ad("adata_posterior_subset_transfer.h5ad")
adata_anterior_subset_transfer.write_h5ad("adata_anterior_subset_transfer.h5ad")
...
for name, adata in [
    ...
    sns.distplot( adata.obs["n_genes_by_counts"][adata.obs["n_genes_by_counts"] < 4000],
        kde=False,bins=60,ax=axs[3],)
    plt.savefig('' + "qc1.distplot_"+name+".png")
...
sc.pp.normalize_total(adata_cortex, inplace=True)
sc.pp.log1p(adata_cortex)
adata_cortex.write_h5ad("data_processed.h5ad")
adata_cortex=sc.read('adata_processed.h5ad')
df1=adata_cortex.obs
df1.groupby('cell_cluster').size()
df1.groupby('cell_subclass').size()


sc.pp.pca(adata_cortex)
sc.pp.neighbors(adata_cortex)
sc.tl.umap(adata_cortex)

sc.pl.umap(adata_cortex,color='cell_subclass')
...
adata_spatial_anterior
adata_spatial_posterior
adatas_anterior = [adata_cortex, adata_spatial_anterior]
adatas_posterior = [adata_cortex,adata_spatial_posterior]
...
cp_posterior_df.index = adata_spatial_posterior.obs.index

adata_anterior_subset_transfer = adata_spatial_anterior.copy()
adata_anterior_subset_transfer.obs = pd.concat(
    [adata_spatial_anterior.obs, cp_anterior_df], axis=1
)

adata_posterior_subset_transfer = adata_spatial_posterior.copy()
adata_posterior_subset_transfer.obs = pd.concat(
    [adata_spatial_posterior.obs, cp_posterior_df], axis=1
)
```
...
sc.pl.spatial(adata_posterior_subset_transfer, 
 ...
 查看基因表达量
sc.pl.spatial(adata_anterior_subset_transfer, 
              img_key="hires", color=[ 'ENSMUSG00000095041',"ENSMUSG00000051951"])
...
adata_spatial_anterior = sc.datasets.visium_sge(
    sample_id="V1_Mouse_Brain_Sagittal_Anterior") # 数据集1
adata_spatial_anterior.write("adata_spatial_anterior.h5ad",compression='gzip')
adata_spatial_anterior=sc.read('data_spatial_anterior.h5ad')
...
plt.tight_layout()
adata_spatial.write("adata_spatial.h5ad",compression='gzip')
